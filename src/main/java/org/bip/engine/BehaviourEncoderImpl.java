...
/**
 * Receives information about the behaviour of each registered component and
...
public class BehaviourEncoderImpl implements BehaviourEncoder {
	...
	public synchronized void createBDDNodes(BIPComponent component, List<Port> componentPorts,
		...
		int initialNoNodes = nbComponentPorts + nbComponentStates + nbVar;

		logger.debug("Going to extend the variables in the BDD");
		logger.debug("Number of vars in the engine {}, initialNoNodes {}", engine.getBDDManager().varNum(),
				initialNoNodes);
		if (engine.getBDDManager().varNum() < initialNoNodes) {
			engine.getBDDManager().setVarNum(initialNoNodes);
		}
		logger.debug("Now we have {} vars in the engine.", engine.getBDDManager().varNum());
		...
		for (int i = 0; i < nbComponentStates; i++) {
			 ...
			 * component instance.
			 */
			singleNodeBDDsForStates[i] = engine.getBDDManager().ithVar(i + nbVar);

			logger.debug("For component {}, state {} has BDD node " + singleNodeBDDsForStates[i], component,
					componentStates.get(i));
			...
			if (singleNodeBDDsForStates[i] == null) {
				try {
					...
					throw new BIPEngineException(
							"Single node BDD for state " + componentStates.get(i) + " is equal to null");
		...
		logger.debug(
				"Behaviour Encoder: Number of component ports: " + nbComponentPorts + " for component: " + component);
		logger.debug(
				"Behaviour Encoder: Number of component states: " + nbComponentStates + " for component: " + component);
		...
		for (int i = 0; i < nbComponentPorts; i++) {
			 ...
			 * component instance.
			 */
			singleNodeBDDsForPorts[i] = engine.getBDDManager().ithVar(i + nbComponentStates + nbVar);

			logger.debug("For component {}, port {} has BDD node " + singleNodeBDDsForPorts[i], component,
					componentPorts.get(i));

			if (singleNodeBDDsForPorts[i] == null) {
				try {
					logger.error("Single node BDD for port {} is equal to null", componentPorts.get(i));
					throw new BIPEngineException(
							"Single node BDD for port " + componentPorts.get(i) + " is equal to null.");
				} catch (BIPEngineException e) {
					e.printStackTrace();
					throw e;
				}
			...
			if (portToBDD.get(componentPorts.get(i).getId()) == null) {
				try {
					...
					throw new BIPEngineException(
							"BDD node that corresponds to the port " + componentPorts.get(i).getId() + " of component "
									+ component.getId() + " is not created.");
				} catch (BIPEngineException e) {
					e.printStackTrace();
					throw e;
				}
		...
		for (String componentState : componentStates) {
			...
			for (String otherState : componentStates) {
				if (!componentState.equals(otherState)) {
					logger.debug("BE: Negated State: " + otherState);
			...
			if (!statePorts.isEmpty()) {
				for (Port port : statePorts) {
					logger.debug("BE: Component state port: " + port);
					...
					for (Port otherPort : componentPorts) {
						if (!port.getId().equals(otherPort.getId())) {
							logger.debug("BE: Negated ports: " + otherPort);
				...
				for (Port otherPort : componentPorts) {
					logger.debug("BE: All negated ports: " + otherPort);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.bip.engine.api.BehaviourEncoder#setEngine(org.bip.engine.api.
	 * BDDBIPEngine)
	 */
	 ...
	 * 
	 * @see
	 * org.bip.engine.api.BehaviourEncoder#setBIPCoordinator(org.bip.engine.api.
	 * BIPCoordinator)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.bip.engine.api.BehaviourEncoder#getBDDOfAPort(org.bip.api.
	 * BIPComponent, java.lang.String)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.bip.engine.api.BehaviourEncoder#getBDDOfAState(org.bip.api.
	 * BIPComponent, java.lang.String)
	 */
	 ...
	 * 
	 * @see
	 * org.bip.engine.api.BehaviourEncoder#getStateToBDDOfAComponent(org.bip.api
	 * .BIPComponent)
	 */
	 ...
	 * 
	 * @see
	 * org.bip.engine.api.BehaviourEncoder#getPortToBDDOfAComponent(org.bip.api.
	 * BIPComponent)
	 */
