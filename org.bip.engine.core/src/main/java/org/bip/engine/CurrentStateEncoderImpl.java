...
/**
 * Receives information about the current state and the list of disabled ports
 * of each registered component and computes the current state BDDs.
 * 
 * @author mavridou
 */
...
public class CurrentStateEncoderImpl implements CurrentStateEncoder {
	...
	/**
	 * Computes the current State BDD. Takes as an argument the current state of
	 * the component and computes the disjunction of the BDD corresponding to
	 * this state with the negation of the BDDs of all the other states of this
	 * component.
	 *
	 * @param component
	 *            the component
	 ...
	 */
	/**
	 * BIP Component informs about its current State and its list of disabled
	 * Ports due to guards. We find the index of the current state of the
	 * component and the index of the disabledPorts to be used to find the
	 * corresponding BDDs by calling the componentCurrentStateBDD().
	 * 
	 * @param BIP
	 *            Component that informs about its current state
	 * @param Name
	 *            of the current state of the component
	 * @param ArrayList
	 *            of the disabled ports of the components
	 * 
	 * @return the current state BDD
	 */
	public synchronized BDD inform(BIPComponent component, String currentState, Set<Port> disabledPorts)
		...
		StringBuilder sb = new StringBuilder("Current state BDD for " + component + "is: "
				+ statesToBDDs.get(currentState) + "(" + currentState + ") ");
		for (String componentState : componentStates) {
			...
			if (!componentState.equals(currentState)) {
				...
				sb.append("&& !" + statesToBDDs.get(componentState) + "(" + componentState + ") ");
			}
		}
		...
		for (Port disabledPort : disabledPorts) {
			...
			sb.append("&& !" + portsToBDDs.get(disabledPort.getId()) + " (" + portsToBDDs.get(disabledPort.getId())
					+ ") ");
	 ...
	 * 
	 * @see
	 * org.bip.engine.api.CurrentStateEncoder#setBIPCoordinator(org.bip.engine.
	 * api.BIPCoordinator)
	 */
	 ...
	 * 
	 * @see
	 * org.bip.engine.api.CurrentStateEncoder#setBehaviourEncoder(org.bip.engine
	 * .api.BehaviourEncoder)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.bip.engine.api.CurrentStateEncoder#setEngine(org.bip.engine.api.
	 * BDDBIPEngine)
	 */
